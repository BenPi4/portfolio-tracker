...
class PortfolioManager:
    ...
    def sign_up(self, username, password, user_email=""):
        ...
        Updates the Users_DB with Username, Password, Sheet_ID, and Email.
        """
        try:
            ...
            # 2. Duplicate the Template file for the new user
            new_sheet = self.client.copy(self.TEMPLATE_ID, title=f"Portfolio_{username}")
            
            # 3. Share the new spreadsheet with the user's email if provided
            if user_email and "@" in user_email:
                new_sheet.share(user_email, perm_type='user', role='writer')
            
            # 4. Append row to DB
            # Order: [Username, Password, Sheet_ID, Email]
